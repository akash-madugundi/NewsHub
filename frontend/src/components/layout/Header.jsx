      ...
      {menuOpen && (
        <div className="md:hidden bg-blue-600 w-full absolute top-16 left-0 z-10 shadow-lg">
          <ul className="flex flex-col items-center space-y-6 py-4">
            <li>
              <Link to="/news" onClick={() => setMenuOpen(false)} className="text-white flex items-center space-x-2">
            ...
            <li className="relative">
              ...
              {dropdownOpen && (
                <div className="absolute left-1/2 transform -translate-x-1/2 mt-2 w-40 bg-white rounded-lg shadow-lg z-10">
                  <Link onClick={() => { setDropdownOpen(false); setMenuOpen(false); }} to="/news/profile" className="block px-4 py-2 text-gray-800 hover:bg-gray-200">My Profile</Link>
